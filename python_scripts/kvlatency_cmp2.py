...
def makePlot(ax, d, ls, xlabel=True, ylabel=True):
  ...
  if xlabel:
    plt.xlabel('Data size (bytes)',  fontsize=defFontSize)
  if ylabel:
    plt.ylabel('Latency ($\mu$sec)', fontsize=defFontSize)
...
def makeDiffPlot(ax, d1, d2, args, ls, xlabel=True, ylabel=True):
  ...
  if xlabel:
    plt.xlabel('Data size (bytes)',fontsize=defFontSize)
  if ylabel:
    plt.ylabel('Ratio',fontsize=defFontSize)
...
def makePanel(d1, d2, args, doX, doY):
  ...
  ax = fig.add_subplot(1,3,args['pl'])
  retick(ax, 14)

  makePlot(ax, d1,  '-', doX, doY)
  makePlot(ax, d2, '--', doX, doY)
  
  plt.legend(args['leg'], loc='upper left',fontsize=9)
  plt.title(args['title'], fontsize=defFontSize)
...
filesn1 = pu.getCmd('ls nval1_normal_' + suffix + 'iter*.txt').replace('\n', ' ')
d1 = getProfilerOutput(filesn1)

filesn3 = pu.getCmd('ls nval3_normal_' + suffix + 'iter*.txt').replace('\n', ' ')
d3 = getProfilerOutput(filesn3)

filesw1c1 = pu.getCmd('ls nval1_w1c_' + suffix + 'iter*.txt').replace('\n', ' ')
dw1c1 = getProfilerOutput(filesw1c1)

filesw1c3 = pu.getCmd('ls nval3_w1c_' + suffix + 'iter*.txt').replace('\n', ' ')
dw1c3 = getProfilerOutput(filesw1c3)
...
args['pl']    = 2
print 'Here 2'
ax = makePanel(d3, dw1c3, args, True, True)
...
args['title'] = 'Riak KV' + suffixstr + 'Round-trip Latencies\nnormal vs. write-once, nval=1, erlang client'
args['rat']   ='w1c/normal'
args['pl']    = 3
print 'Here 2'
ax = makePanel(d1, dw1c1, args, True, True)

  
fig.savefig('kvlatency_cmp2.png')
plt.show()
